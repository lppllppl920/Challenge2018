...
def validation_multi(model, criterion, valid_loader, num_classes, first_time, class_color_table, result_root):
    with torch.no_grad():
        model.eval()
        losses = []
        ...
        for inputs, targets in valid_loader:
            ...
            if first_time:
                utils.write_images(target_classes, class_color_table, result_root, "gt_", count)
            count = utils.write_images(output_classes, class_color_table, result_root, "pred_", count)
        ...
        print(
            'Valid loss: {:.4f}, average IoU: {:.4f}, average Dice: {:.4f}'.format(valid_loss, average_iou,
                                                                                   average_dices))
